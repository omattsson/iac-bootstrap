...
| `references/` | Canonical `.tmpl` template files with `{{PLACEHOLDER}}` syntax. This is the single source of truth — edit templates here. |
...
| `cli/bootstrap_iac/templates/` | Generated copy of the templates, bundled with the CLI package. Not tracked in git. Do not edit — run `python scripts/build_templates.py` to regenerate it from `references/`. |
| `scripts/build_templates.py` | Generates the bundled template copy from `references/` (`--check` verifies it is up to date). |
| `scripts/validate_templates.py` | CI validator: checks placeholder syntax, YAML frontmatter, example cleanliness, SKILL.md references, and that the bundle generates faithfully from `references/`. |
...
1. **Template validation** (`scripts/validate_templates.py`) — checks placeholder syntax, YAML frontmatter, example cleanliness, SKILL.md references, and that the bundle generates faithfully from `references/`.
2. **CLI tests** — generates the template bundle, installs the CLI package, verifies the entry point, and runs `pytest cli/tests/`.
...
3. **Templates have one source of truth** — edit templates in `references/` only. `cli/bootstrap_iac/templates/` is generated by `scripts/build_templates.py` and is not tracked in git; do not suggest editing it or committing it.
