...
| 12 | Write `tests/plan_test.tftest.hcl` | Look up test framework syntax |
...
| # | Step | Notes |
|---|------|-------|
| 1 | Ask agent: *"Create a Key Vault module following our conventions"* | Agent reads `CLAUDE.md` / `copilot-instructions.md` for all conventions |
...
| 8 | Update tests to cover the fixed behaviour | Easy to skip under time pressure |
...
- **Knowledge isn't locked in individuals**: Tribal knowledge encoded in `copilot-instructions.md` and `CLAUDE.md` is available to every team member.
