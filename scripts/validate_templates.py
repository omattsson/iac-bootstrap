...
# Files with expected YAML frontmatter. references/ is the canonical source;
# the bundled cli/bootstrap_iac/templates/ copy is generated, so it is not
# validated here.
FRONTMATTER_GLOBS = [
...
def check_placeholder_syntax() -> list[str]:
    ...
    Only the canonical references/ templates are checked; the bundled copy is
    generated from them, and build artifacts are ignored.
    """
    errors: list[str] = []
    tmpl_files = list(REFERENCES_DIR.rglob("*.tmpl"))
    if not tmpl_files:
        errors.append("ERROR: No .tmpl files found under references/.")
        return errors
...
def check_template_generation() -> list[str]:
    """Verify the bundled templates generate faithfully from references/.

    references/ is the single source of truth. The bundled copy under
    cli/bootstrap_iac/templates/ is generated by scripts/build_templates.py.
    This regenerates into a temporary directory and confirms the generator
    reproduces every reference template exactly, so a clean checkout passes
    ...
    try:
        ...
        return [f"cannot import build_templates: {exc}"]
...
def main() -> int:
    ...
    checks = [
        ...
        ("Bundled templates generate faithfully from references/", check_template_generation),
    ]
