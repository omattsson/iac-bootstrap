...
## Hierarchy Files
- `account.hcl` — account_id, module_tags, OIDC role config
- `region.hcl` — aws_region, availability_zones
- `stack.hcl` — stack name, prefix
- `root.hcl` — remote state (S3), provider generation, global input merge
...
## Module Sources
`git::https://github.com/acme-infra/tf-module-{name}?ref=${local.module_tags.{name}}`
Version tags from `account.hcl`. Never hardcode versions in component files.
