...
## Hierarchy Files
- `subscription.hcl` — subscription_id, module_tags, Workload Identity config
- `site.hcl` — location, site_name
- `stack.hcl` — stack name, prefix
- `root.hcl` — remote state (Azure Blob), provider generation, global input merge
...
## Module Sources
`git::https://gitlab.com/globex-infra/tf-module-{name}?ref=${local.module_tags.{name}}`
