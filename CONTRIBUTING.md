...
Examples live in `examples/`. There are two kinds — see `examples/README.md`.

**Reproducible examples (preferred)** live under `examples/generated/` and are
generated by the CLI from a committed `.bootstrap-iac.yaml`. CI verifies them.

1. Create `examples/generated/<name>/.bootstrap-iac.yaml` with the config.
2. Generate the output: `python scripts/build_examples.py`.
3. Commit the config and the generated files.
4. After any template or generator change, run `python scripts/build_examples.py`
   and commit the result. CI fails when an example is stale
   (`python scripts/build_examples.py --check`).

**Illustrative examples** (other `examples/*`) are hand-authored SKILL output.
Replace all `{{PLACEHOLDER}}` values with realistic values — no placeholders
should remain. Document any output the CLI does not generate in
`examples/README.md`.
